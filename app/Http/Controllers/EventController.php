...
class EventController extends Controller
{
    public function welcome() {
        ...
        if(isset($reservetionCount)){
            return view('welcome', compact('events', 'reservetionCount'));
        }
    ...
    public function addEvent(EventReuest $request){
        ...
        return redirect()->back()->with('add', 'seccessfully added');
    }
    ...
    public function showDetails($id){
        ...
        if(isset($reservetionCount)){
            return view('events.eventDetails', compact('events', 'organisateur', 'reservetionCount'));
        }
    ...
    public function editEvent(EventReuest $request, $id){
        ...
        return redirect()->back()->with('update', 'seccessfully updated');
    }
    ...
    public function myEvents(){
        ...
        // dd($id_organisateur);
        $myEvents = Event::where('validation', "valid")->where('status', "available")->where('id_organisateur', $id_organisateur)->with('organisateurs', 'category')->paginate(9);
        return view('events.myEvents', compact('myEvents', 'id_organisateur'));
    }
    
    public function search(Request $request){
        $search = $request->input('search');
        $eventsResult = Event::where('titre', 'like', "%$search%")->where('validation', "valid")->where('status', "available")->with('organisateurs', 'category')->paginate(9);
        $id_organisateur = Auth::user()->id;
    ...
    public function filtre($id){
        ...
        $food = Categorie::where('category_name', 'Food Festivals')->get();
        $workshops = Categorie::where('category_name', 'Workshops')->get();
        return view('events.filtre', compact('eventsResult', 'categories', 'music', 'art', 'food', 'workshops', 'food', 'health'));
    }
    ...
    public function filtreDate($date){
        ...
        $food = Categorie::where('category_name', 'Food Festivals')->get();
        $workshops = Categorie::where('category_name', 'Workshops')->get();
        return view('events.filtreDate', compact('eventsResult', 'categories', 'music', 'art', 'food', 'workshops', 'food', 'health'));
    }
    ...
    public function statisticsReservation($id){
        ...
        $organisateur = auth::user()->id;
        return view('events.statistics', compact('events', 'organisateur','reservationCount', 'reservetionCount', 'validReaervationCount', 'invalidReaervationCount'));
    }
    ...
    public function statisticsEvents(){
        ...
        return view('events.statisticsEvents', compact('invalidEvents', 'validEvents', 'availableEvents', 'unavailableEvents', 'usersCount'));
    }
}